...
const navItems = [
  ...
  { name: 'Clientes', path: '/dashboard/clientes', icon: UserCheck, roles: ['superadmin','admin_empresa'] }, // NUEVO
];
  ...
  <div className={`fixed inset-y-0 left-0 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'}
      ...
      {navItems.map((item) => {
        ...
        return (
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center px-4 py-2 text-sm font-medium rounded-lg transition duration-150 focus:outline-none focus:ring-2 focus:ring-blue-500 ${
                isActive
    ...
    <div className="p-4 border-t border-gray-700 space-y-1">
      ...
      <button
        ...
        className="flex items-center px-4 py-2 text-sm font-medium rounded-lg w-full text-red-400 hover:bg-gray-700 hover:text-red-300 transition duration-150 focus:outline-none focus:ring-2 focus:ring-red-500 rounded"
      >
        ...
        {isDropdownOpen && (
          <div className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl z-40 py-1 border" role="menu">
            <div className="px-4 py-2 text-sm text-gray-900 border-b mb-1 font-semibold">
              {usuario?.email}
              <p className="text-xs text-blue-600 capitalize">Rol: {usuario?.rol?.replace('_',' ')}</p>
            </div>
...
const DashboardLayout = ({ onLogout }) => {
  ...
  const hasAccess = useCallback((requiredRoles) => {
    ...
    return requiredRoles.includes(usuario.rol.toLowerCase());
  }, [usuario]);
  ...
  return (
    <div className="flex h-screen bg-gray-100">
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-black opacity-50 z-20 md:hidden" onClick={closeSidebar} aria-hidden="true" role="presentation" />
      )}
